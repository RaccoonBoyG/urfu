...
from urfu import exceptions, fmt, hooks, plugins
from urfu.plugins import v0 as plugins_v0
...
class PluginsTests(PluginsTestCase):
    ...
    def test_load(self) -> None:
        config: Config = {urfu_config.PLUGINS_CONFIG_KEY: []}
        ...
        self.assertEqual(
            ["plugin1", "plugin2"], config[urfu_config.PLUGINS_CONFIG_KEY]
        )
    ...
    def test_enable_twice(self) -> None:
        ...
        config: Config = {urfu_config.PLUGINS_CONFIG_KEY: []}
        urfu_config.save_enabled_plugins(config)
        self.assertEqual(["plugin1"], config[urfu_config.PLUGINS_CONFIG_KEY])

    def test_load_not_installed_plugin(self) -> None:
        self.assertRaises(exceptions.UrfuError, plugins.load, "plugin1")
    ...
    def test_disable(self) -> None:
        ...
        urfu_config.enable_plugins(config)
    ...
    def test_config_disable_plugin(self) -> None:
        ...
        with temporary_root() as root:
            config = urfu_config.load_minimal(root)
            config_pre = config.copy()
            with patch.object(fmt, "STDOUT"):
                hooks.Actions.PLUGIN_UNLOADED.do("plugin1", "", config)
            config_post = urfu_config.load_minimal(root)
