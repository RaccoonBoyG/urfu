"""
List of all the action, filter and context names used across Urfu. This module is used
to generate part of the reference documentation.
"""
from __future__ import annotations

# The Urfu plugin system is licensed under the terms of the Apache 2.0 license.
...
class Actions:
    """
    This class is a container for all actions used across Urfu (see
    :py:class:`urfu.core.hooks.Action`). Actions are used to trigger callback functions at
    specific moments in the Urfu life cycle.
    ...
    Then create your callback function and decorate it with the :py:meth:`add <urfu.core.hooks.Action.add>` method of the
    ...
    is called, i.e: when ``urfu local start`` or ``urfu dev start`` is run.

    Note that action callbacks do not return anything.

    For more information about how actions work, check out the :py:class:`urfu.core.hooks.Action` API.
    """
    ...
    #:
    #: - Python packages that declare a "urfu.plugin.v0" entrypoint.
    #: - Python packages that declare a "urfu.plugin.v1" entrypoint.
    #: - YAML and Python plugins stored in ~/.local/share/urfu-plugins (as indicated by ``urfu plugins printroot``)
    #: - When running the binary version of Urfu, official plugins that ship with the binary are automatically discovered.
    #:
    #: Discovering a plugin is typically done by the Urfu plugin mechanism. Thus, plugin
    ...
    #: Called as soon as we have access to the Urfu project root.
    #:
    #: :parameter str root: absolute path to the project root.
...
class Filters:
    """
    Here are the names of all filters used across Urfu. (see
    :py:class:`urfu.core.hooks.Filter`) Filters are used to modify some data at
    specific points during the Urfu life cycle.
    ...
    <urfu.core.hooks.Filter.add>` method of the filter instance you need::
    ...
    #: List of folders to bind-mount in docker-compose containers, either in ``urfu local`` or ``urfu dev``.
    #:
    #: Many ``urfu local`` and ``urfu dev`` commands support ``--mounts`` options
    ...
    #: ``config.yml`` file. Default settings may be overridden with ``urfu config save --set=...``, in which
    ...
    #: Modify existing settings, either from Urfu core or from other plugins. Beware not to override any
    ...
    #:   filter to modify the Urfu templates.
    ...
    #:    {{ "somevalue"|my_filter }}
    #:
    #: Note that Jinja2 filters are a completely different thing than the Urfu hook
    #: filters, although they share the same name.
    #:
    #: Out of the box, Urfu comes with the following filters:
    #:
    ...
    #: List of images to be built when we run ``urfu images build ...``.
    #:
    #: :parameter list[tuple[str, tuple[str, ...], str, tuple[str, ...]]] tasks: list of ``(name, path, tag, args)`` tuples.
    #:
    #:    - ``name`` is the name of the image, as in ``urfu images build myimage``.
    ...
    #:      be ``"{{ DOCKER_REGISTRY }}/myimage:{{ URFU_VERSION }}"``.
    ...
    #:
    #:    - ``name`` is the name of the image, as in ``urfu images pull myimage``.
    ...
    #: List of images to be pushed when we run ``urfu images push ...``.
    #: Parameters are the same as for :py:data:`IMAGES_PULL`.
    IMAGES_PUSH: Filter[list[tuple[str, str]], [Config]] = filters.get("images:push")

    #: List of plugin indexes that are loaded when we run `urfu plugins update`. By
...
class Contexts:
    """
    Here we list all the :py:class:`contexts <urfu.core.hooks.Context>` that are used across Urfu. It is not expected that
