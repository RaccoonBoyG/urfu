...
from urfu import env, exceptions, fmt, hooks, serialize
...
class BasePlugin:
    """
    Urfu plugins are defined by a name and an object that implements one or more of the
    ...
    Urfu templates. For instance, to add "somecontent" to a template that includes '{{
    ...
    `command` (click.Command): if a plugin exposes a `command` attribute, users will be able to run it from the command line as `urfu pluginname`.
    """
...
class EntrypointPlugin(BasePlugin):
    """
    Entrypoint plugins are regular python packages that have a 'urfu.plugin.v0' entrypoint.

    The API for Urfu plugins is currently in development. The entrypoint will switch to
    'urfu.plugin.v1' once it is stabilised.
    """

    ENTRYPOINT = "urfu.plugin.v0"
    ...
    def _version(self) -> t.Optional[str]:
        if not self.loader.dist:
            raise exceptions.UrfuError(f"Entrypoint plugin '{self.name}' has no dist.")
...
class OfficialPlugin(BasePlugin):
    ...
    def _load_obj(self) -> None:
        self.obj = importlib.import_module(f"urfu{self.name}.plugin")

    def _version(self) -> t.Optional[str]:
        try:
            module = importlib.import_module(f"urfu{self.name}.__about__")
    ...
    def discover_all(cls) -> None:
        ...
        for plugin_name in cls.NAMES:
            if importlib.util.find_spec(f"urfu{plugin_name}") is not None:
...
def _discover_v0_plugins() -> None:
    ...
    to be discovered manually from within the urfu binary.

    Installing entrypoint or dict plugins can be disabled by defining the
    ``URFU_IGNORE_DICT_PLUGINS`` and ``URFU_IGNORE_ENTRYPOINT_PLUGINS``
    environment variables.
    """
    with hooks.Contexts.PLUGINS.enter():
        if "URFU_IGNORE_ENTRYPOINT_PLUGINS" not in os.environ:
            with hooks.Contexts.PLUGINS_V0_ENTRYPOINT.enter():
                EntrypointPlugin.discover_all()
        if "URFU_IGNORE_DICT_PLUGINS" not in os.environ:
