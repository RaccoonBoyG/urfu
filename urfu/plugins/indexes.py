...
from urfu import env, fmt, hooks, serialize, utils
from urfu.__about__ import __version__, __version_suffix__
...
def parse_index(content: str) -> list[dict[str, str]]:
    try:
        ...
        raise UrfuError(f"Could not parse index: {e}") from e
...
def find_in_cache(name: str) -> IndexEntry:
    ...
    for entry in iter_cache_entries():
        if entry.name == name:
            return entry
    raise UrfuError(f"Plugin '{name}' could not be found in indexes")
...
def load_cache() -> list[dict[str, str]]:
    try:
        ...
        raise UrfuError(
            f"Local index cache could not be found in {Indexes.CACHE_PATH}. Run `urfu plugins update`."
        ) from e
