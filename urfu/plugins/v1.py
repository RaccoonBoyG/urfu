...
def _discover_entrypoint_plugins() -> None:
    """
    Discover all plugins that declare a "urfu.plugin.v1" entrypoint.
    """
    with hooks.Contexts.PLUGINS.enter():
        if "URFU_IGNORE_ENTRYPOINT_PLUGINS" not in os.environ:
            for entrypoint in pkg_resources.iter_entry_points("urfu.plugin.v1"):
...
def discover_module(path: str) -> None:
    ...
    def load() -> None:
        ...
        spec = importlib.util.spec_from_file_location("urfu.plugin.v1.{name}", path)
