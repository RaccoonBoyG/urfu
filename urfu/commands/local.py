...
from urfu.commands.upgrade.local import upgrade_from
...
class LocalTaskRunner(compose.ComposeTaskRunner):
    def __init__(self, root: str, config: Config):
        ...
        docker_compose_tmp_path = urfu_env.pathjoin(
            self.root, "local", "docker-compose.tmp.yml"
        )
        docker_compose_jobs_tmp_path = urfu_env.pathjoin(
...
Urfu may not work if Docker is configured with < 4 GB RAM. Please follow instructions from:
    ...
    if run_upgrade_from_release is not None:
        ...
        if not non_interactive:
            to_release = urfu_env.get_current_open_edx_release_name()
...
It is strongly recommended to make a backup before upgrading. To do so, run:
    ...
    urfu local stop
    sudo rsync -avr "$(urfu config printroot)"/ /tmp/urfu-backup/

In case of problem, to restore your backup you will then have to run: sudo rsync -avr /tmp/urfu-backup/ "$(urfu config printroot)"/
...
If you run custom Docker images, you must rebuild them now by running the following command in a different shell:
    ...
    urfu images build all # list your custom images here
...
def upgrade(context: click.Context, from_release: t.Optional[str]) -> None:
    fmt.echo_alert(
        ...
        "To perform a full upgrade, you should run `urfu local launch`."
    )
