...
from urfu import exceptions, fmt, hooks
from urfu import interactive as interactive_config
from urfu import serialize, utils
from urfu.commands import jobs
from urfu.commands.config import save as config_save_command
from urfu.commands.context import BaseTaskContext
from urfu.commands.upgrade import OPENEDX_RELEASE_NAMES
from urfu.commands.upgrade.k8s import upgrade_from
...
class K8sTaskRunner(BaseTaskRunner):
    ...
    Note: a single Urfu "task" correspond to a Kubernetes "job":
    https://kubernetes.io/docs/concepts/workloads/controllers/job/
    A Urfu "job" is composed of multiple Urfu tasks run in different services.
    ...
    def run_task(self, service: str, command: str) -> int:
        ...
        with open(
            urfu_env.pathjoin(self.root, "k8s", "jobs.yml"), "w", encoding="utf-8"
        ) as job_file:
    ...
    def _find_job(self, name: str, all_jobs: Iterable[Any]) -> Any:
        ...
        for job in all_jobs:
            ...
            if job_name == name:
                return job
        raise exceptions.UrfuError(f"Could not find job '{name}'")
...
def launch(context: click.Context, non_interactive: bool) -> None:
    run_upgrade_from_release = urfu_env.should_upgrade_from_release(context.obj.root)
    if run_upgrade_from_release is not None:
        click.echo(fmt.title("Upgrading from an older release"))
        context.invoke(
            upgrade,
            from_release=urfu_env.get_env_release(context.obj.root),
        )
...
commands in a different shell:
    ...
    urfu images build all # add your custom images here
    urfu images push all
...
def delete(context: K8sContext, yes: bool) -> None:
    ...
    utils.kubectl(
        "delete",
        "-k",
        urfu_env.pathjoin(context.root),
...
def do(context: K8sContext) -> None:
    ...
    def _start_base_deployments(_job_name: str, *_args: Any, **_kwargs: Any) -> None:
        ...
        whenever we run `urfu k8s do <job> --help`.
        """
        config = urfu_config.load(context.root)
        wait_for_deployment_ready(config, "caddy")
        for name in ["elasticsearch", "mysql", "mongodb"]:
            if urfu_config.is_service_activated(config, name):
...
def upgrade(context: click.Context, from_release: Optional[str]) -> None:
    ...
    if from_release is None:
        ...
        fmt.echo_alert(
            ...
            "To perform a full upgrade, you should run `urfu k8s launch`."
        )
...
def kubectl_apply(root: str, *args: str) -> None:
    utils.kubectl("apply", "--kustomize", urfu_env.pathjoin(root), *args)
