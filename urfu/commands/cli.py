...
from urfu import exceptions, fmt, hooks, utils
from urfu.__about__ import __app__, __version__
from urfu.commands.config import config_command
from urfu.commands.context import Context
from urfu.commands.dev import dev
from urfu.commands.images import images_command
from urfu.commands.k8s import k8s
from urfu.commands.local import local
from urfu.commands.mounts import mounts_command
from urfu.commands.plugins import plugins_command
...
class UrfuCli(click.MultiCommand):
    """
    ...
    def list_commands(self, ctx: click.Context) -> list[str]:
        ...
        - shell autocompletion: urfu <tab>
        - print help: urfu, urfu -h
        """
        return sorted(
    ...
    def get_command(
        ...
        """
        This is run when passing a command from the CLI. E.g: urfu config ...
        """
...
@click.group(
    ...
    help="Urfu is the Docker-based Open edX distribution designed for peace of mind.",
)
...
@click.option(
    ...
    help="Root project directory (environment variable: URFU_ROOT)",
)
@click.option(
    "-h",
    "--help",
...
def cli(context: click.Context, root: str, show_help: bool) -> None:
    if utils.is_root():
        fmt.echo_alert(
            "You are running Urfu as root. This is strongly not recommended. If you are doing this in order to access"
