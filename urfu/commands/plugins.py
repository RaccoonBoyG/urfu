...
from urfu import exceptions, fmt, hooks, plugins, utils
from urfu.plugins import indexes
from urfu.plugins.base import PLUGINS_ROOT, PLUGINS_ROOT_ENV_VAR_NAME
...
@click.group(
    name="plugins",
    short_help="Manage Urfu plugins",
)
def plugins_command() -> None:
    """
    Manage Urfu plugins to add new features and customise your Open edX platform.

    Plugins can be downloaded from local and remote indexes. See the `urfu plugins
    index` subcommand.

    After the plugin index cache has been updated, plugins can be searched with:

        urfu plugins search <pattern>

    Plugins are installed with:

        urfu plugins install <name>
    """
...
def enable(context: Context, plugin_names: list[str]) -> None:
    config = urfu_config.load_minimal(context.root)
    for plugin in plugin_names:
        plugins.load(plugin)
        fmt.echo_info(f"Plugin {plugin} enabled")
    urfu_config.save_enabled_plugins(config)
    urfu_config.save_config_file(context.root, config)
    fmt.echo_info(
        "You should now re-generate your environment with `urfu config save`."
    )
...
def disable(context: Context, plugin_names: list[str]) -> None:
    ...
    disabled: list[str] = []
    for plugin in urfu_config.get_enabled_plugins(config):
...
def update(context: Context) -> None:
    ...
    """
    config = urfu_config.load(context.root)
...
def install(names: list[str]) -> None:
    ...
    1. A plugin name from the plugin indexes (see `urfu plugins search`)
    2. A local file that will be copied to the plugins root
    3. An http(s) location that will be downloaded to the plugins root

    In cases 2. and 3., the plugin root corresponds to the path given by `urfu plugins
    printroot`.
    """
...
def find_and_install(names: list[str], pip_install_opts: t.List[str]) -> None:
    ...
    if pip_requirements:
        ...
        with tempfile.NamedTemporaryFile(
            prefix="urfu-reqs-", suffix=".txt", mode="w"
        ) as tmp_reqs:
...
def index_command() -> None:
    ...
    A plugin index is a list of Urfu plugins. An index can be public and shared with
    the community, or private, for instance to share plugins with a select group of
    users. Plugin indexes are a great way to share your plugins with other Urfu users.
    ...
    Plugin index cache is stored locally in the following subdirectory of the Urfu project environment:
