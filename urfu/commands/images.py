...
from urfu import exceptions, fmt, hooks, images, utils
from urfu.commands.context import Context
from urfu.commands.params import ConfigLoaderParam
from urfu.core.hooks import Filter
...
def _add_core_images_to_build(
    ...
    """
    Add base images to the list of Docker images to build on `urfu build all`.
    """
    for image, tag in BASE_IMAGE_NAMES:
        build_images.append(
            (
                ...
                urfu_config.get_typed(config, tag, str),
                (),
            )
        )
    ...
    build_images.append(
        (
            ...
            urfu_config.get_typed(config, "DOCKER_IMAGE_OPENEDX_DEV", str),
            (
                "--target=development",
...
def _add_images_to_pull(
    ...
    """
    Add base and vendor images to the list of Docker images to pull on `urfu pull all`.
    """
    vendor_images = [
    ...
    for image, tag_name in vendor_images:
        if config.get(f"RUN_{image.upper()}", True):
            remote_images.append((image, urfu_config.get_typed(config, tag_name, str)))
    for image, tag in BASE_IMAGE_NAMES:
        remote_images.append((image, urfu_config.get_typed(config, tag, str)))
...
def _add_core_images_to_push(
    ...
    """
    Add base images to the list of Docker images to push on `urfu push all`.
    """
    for image, tag in BASE_IMAGE_NAMES:
        remote_images.append((image, urfu_config.get_typed(config, tag, str)))
    return remote_images
...
def build(
    ...
    for image in image_names:
        for name, path, tag, custom_args in find_images_to_build(config, image):
            ...
            # Build
            images.build(
                urfu_env.pathjoin(context.root, path),
                tag,
                *image_build_args,
            )
...
def pull(context: Context, image_names: list[str]) -> None:
    config = urfu_config.load_full(context.root)
    for image in image_names:
...
def find_images_to_build(
    ...
    for name, path, tag, args in hooks.Filters.IMAGES_BUILD.iterate(config):
        ...
        if image in [name, "all"]:
            found = True
            tag = urfu_env.render_str(config, tag)
...
def find_remote_image_tags(
    ...
    for name, tag in all_remote_images:
        if image in [name, "all"]:
            found = True
            yield urfu_env.render_str(config, tag)
    if not found:
        raise ImageNotFoundError(image)


class ImageNotFoundError(exceptions.UrfuError):
