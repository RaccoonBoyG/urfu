...
def ask_questions(config: Config, run_for_prod: Optional[bool] = None) -> None:
    ...
    if run_for_prod is None:
        run_for_prod = config.get("LMS_HOST") != "local.urfu.online.ru"
        run_for_prod = click.confirm(
            fmt.question(
                "Are you configuring a production platform? "
                "Type 'n' if you are just testing Urfu on your local computer"
            ),
            prompt_suffix=" ",
    ...
    if not run_for_prod:
        dev_values: Config = {
            "LMS_HOST": "local.urfu.online.ru",
            "CMS_HOST": "studio.local.urfu.online.ru",
            "ENABLE_HTTPS": False,
        }
        fmt.echo_info(
