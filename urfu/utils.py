...
def is_root() -> bool:
    """
    Check whether urfu is being run as root/sudo.
    """
...
def _docker_compose_command() -> Tuple[str, ...]:
    ...
    """
    if os.environ.get("URFU_USE_COMPOSE_SUBCOMMAND") is not None:
...
def execute_silent(*command: str) -> int:
    with subprocess.Popen(command) as p:
        try:
            ...
            p.kill()
            p.wait()
            raise exceptions.UrfuError(f"Command failed: {' '.join(command)}") from e
        if result > 0:
            raise exceptions.UrfuError(
...
def check_output(*command: str) -> bytes:
    ...
    try:
        ...
        raise exceptions.UrfuError(f"Command failed: {literal_command}") from e
...
def check_macos_docker_memory() -> None:
    ...
    allocated memory. Will raise UrfuError in case of parsing/loading error.
    """
    ...
    try:
        ...
        raise exceptions.UrfuError(f"Error accessing Docker settings file: {e}") from e
...
def read_url(url: str) -> str:
    ...
    if is_http(url):
        ...
        try:
            ...
            raise exceptions.UrfuError(f"Request error: {e}") from e
    ...
    try:
        ...
        raise exceptions.UrfuError(f"File could not be found: {e}") from e
    except UnicodeDecodeError as e:
        raise exceptions.UrfuError(f"File must be encoded as utf8: {e}") from e
