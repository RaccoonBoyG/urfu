...
from urfu.types import Config, ConfigValue

TEMPLATES_ROOT = pkg_resources.resource_filename("urfu", "templates")
...
def _prepare_environment() -> None:
    ...
    hooks.Filters.ENV_TEMPLATE_VARIABLES.add_items(
        [
            ...
            ("URFU_APP", __app__.replace("-", "_")),
            ("URFU_VERSION", __version__),
...
class Renderer:
    ...
    def render_str(self, text: str) -> str:
        try:
            ...
            raise exceptions.UrfuError(f"Template syntax error: {e.args[0]}")
    ...
    def render_template(self, template_name: str) -> t.Union[str, bytes]:
        ...
        try:
            return self.__render(template)
        except (jinja2.exceptions.TemplateError, exceptions.UrfuError):
    ...
    def __render(self, template: jinja2.Template) -> str:
        try:
            ...
            raise exceptions.UrfuError(f"Missing configuration value: {e.args[0]}")
...
def check_is_up_to_date(root: str) -> None:
    if not is_up_to_date(root):
        fmt.echo_alert(
            ...
            f"v{current_version(root)} while the 'urfu' binary is at v{__version__}. You should upgrade "
...
def is_up_to_date(root: str) -> bool:
    """
    Check if the currently rendered version is equal to the current urfu version.
    """
...
def read_core_template_file(*path: str) -> str:
    """
    Read raw content of template located in urfu core template directory.
    """
