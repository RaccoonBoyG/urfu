...
from urfu import env, exceptions, fmt, hooks, plugins, serialize, utils
from urfu.types import Config, ConfigValue, cast_config, get_typed
...
def load(root: str) -> Config:
    ...
    if not os.path.exists(config_path(root)):
        raise exceptions.UrfuError(
            ...
            "configuration with `urfu config save --interactive` or `urfu local "
...
def get_user(root: str) -> Config:
    """
    Get the user configuration from the urfu root.
...
def convert_json2yml(root: str) -> None:
    """
    Older versions of urfu used to have json config files.
    """
